---
title: "Summary of eQTL associations on chr3 GWAS locus"
author: "Silva Kasela"
date: "7/20/2020"
...
knitr::opts_chunk$set(echo = FALSE, fig.path = here::here("eqtl_coloc", "fig_summary_coloc/"), dev = c('png', 'pdf'))
...
* COVID19 HGI: ANA_C2_V2 - covid vs. population GWAS summary statistics
...
* Coloc-cond/mask
    ...
    + Conditioning used for GTEx v8 with LD data from GTEx, masking used for eQTL catalogue data with LD from 1000G CEU population
...
tabix_paths$id <- sapply(tabix_paths$id, function(x) sub("Fairfax_2014_", "Fairfax_2014_monocyte_", x))

# coloc-standard -------------------------
# Results of coloc
res <- read.table(here("eqtl_coloc", "summary_coloc_eQTL_catalogue.txt"), header = T, sep = "\t", stringsAsFactors = F)
res$id <- sapply(res$eqtl_data, function(x) sub("microarray_", "", x))
...
# coloc-condmask -------------------------
# Results of coloc
res <- read.table(here("eqtl_coloc", "summary_coloc_eQTL_catalogue_condmask.txt"), header = T, sep = "\t", stringsAsFactors = F)
res$id <- sapply(res$eqtl_data, function(x) sub("microarray_", "", x))
res$id_match <- paste0(res$eqtl_data, res$eqtl_gene_name)
...
res <- read.table(here("eqtl_coloc", "summary_coloc_GTEx_v8.txt"), header = T, sep = "\t", stringsAsFactors = F)
...
# coloc-condmask ------------
# Results of coloc - GTEX v8
res <- read.table(here("eqtl_coloc", "summary_coloc_GTEx_v8_condmask.txt"), header = T, sep = "\t", stringsAsFactors = F)
res$id <- sapply(res$eqtl_data, function(x) sub("GTEx_v8_ge_", "", x))
res$id_match <- paste0(res$eqtl_data, res$eqtl_gene_name)
...
What's the eQTL P-value of the lead eVariant and lead GWAS variant (3:45867022:C:G) in various studies from the eQTL Catalogue and GTEx?
...
trait_annot <- tabix_paths[, c("id", "tissue_label")]
rownames(trait_annot) <- trait_annot$id
...
```{r fig-summary-coloc-eqtl-catalogue, fig.width=15, fig.height=11}
i <- 1
res <- res_list[[i]]
...
res %>%
  ...
  select(-min_lead_gwas_variant_eqtl_pval,
         -id) %>% 
  mutate(lead_gwas_pval = as.character(signif(lead_gwas_pval, 4)),
...
```{r fig-summary-coloc-gtex, fig.width=15, fig.height=11}
i <- 2
res <- res_list[[i]]
...
stand$id <- NULL
stand$id_match <- NULL
# there's three cases where the best probe (= min lead_gwas_eqtl_pval) was not used in coloc test
...
write.table(out, file = here("eqtl_coloc", "suppl_table_coloc_results.txt"), col.names = T, row.names = F, sep = "\t", quote = F)
```
